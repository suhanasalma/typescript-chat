...
const Message = ({ message, showDeleteModal, toggleEmojiPicker, isOpenEmojiPicker, setOpenEmojiMessageId, openLightbox }: MessageProps) => {
    ...
    return (
        <div
            key={message._id}
            className={`flex items-start gap-5 my-2 lg:w-9/12 group relative ${message.sender._id === user._id
                ? " justify-end self-end "
                : "self-start justify-start"
                }`}
        >
            <div className="flex items-end justify-between gap-5">
                {message.sender._id !== user._id && <div className="min-w-max">
                    <img onClick={() => openLightbox(message._id)}
                        src={senderImg ? senderImg : userImage}
                        className="w-8 h-8 rounded-full object-cover object-top"
                        alt=""
                    />
                </div>}
                {message?.sender._id !== user._id && <div className='absolute z-10' style={{ left: emojiPickerPosition.left, top: emojiPickerPosition.top }}>
                    {
                ...
                {message?.sender._id === user._id && <div className='absolute z-10' style={{ right: emojiPickerPosition.left, top: emojiPickerPosition.top }}>
                    {
                        isOpenEmojiPicker && <div onClick={handleLightboxClick}>
                            <EmojiPicker style={{ width: "max-content" }} onEmojiClick={onEmojiClick} />
                        </div>
                ...
                {message.sender._id === user._id && <div className={`flex items-center gap-2 `}>
                ...
                >
                    ...
                    <div className="text-[10px] text-slate flex items-center justify-end ">
                        ...
                        {message.sender._id === user._id && <p className="">
                            {msgReceived ? (
                                <IoCheckmarkDoneSharp
                                    className={`${msgRead && "text-blue text-base font-bold"}`}
                                />
                            ) : (
                                <IoCheckmarkOutline />
                            )}
                ...
                {message.sender._id !== user._id && <div className={`flex items-center gap-2 group-hover:opacity-100 opacity-0 duration-300 ease-in-out ${isOpenEmojiPicker && "opacity-100"}`}><p className='p-1 rounded-full bg-white msgEmoji'><FaRegSmile onClick={() => toggleEmojiPicker(message._id)} className='cursor-pointer text-gray' /></p></div>}
                {message.sender._id === user._id && <div className="min-w-max flex justify-end">
                    <img onClick={() => openLightbox(message._id)}
